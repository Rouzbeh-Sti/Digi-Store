...
export default function AdminUserManagement({ users, onToggleBan, onRoleChange, onInspect }) {
  ...
  const headers = ["نام کاربر", "ایمیل", "سطح دسترسی (نقش)", "وضعیت اشتراک فعال", "وضعیت حساب", "عملیات اداری"];
  ...
  const renderRowCells = (u) => {
    const activeSub = u.subscriptions && u.subscriptions.length > 0 ? u.subscriptions[0] : null;
    ...
    return [
      ...
      // Dynamic Subscription Status Cell
      <div className="flex flex-col">
        {activeSub ? (
          <>
            <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 px-2 py-1 rounded w-fit mb-1 border border-emerald-100">
              {activeSub.plan.title}
            </span>
            <span className="text-[9px] font-bold text-gray-400">انقضا: {new Date(activeSub.endDate).toLocaleDateString('fa-IR')}</span>
          </>
        ) : (
          <span className="text-[10px] font-bold text-gray-400">ندارد</span>
        )}
      </div>,
